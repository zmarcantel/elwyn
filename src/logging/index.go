...
// Opens files and manages the Golang logger interface
//
func Initialize(path string, quiet bool) (logger *Router, err error) {
	...
	if path != "" {
		// create standard log
		file, err = OpenFile(filepath.Join(path, "elwyn.log"))
		if err != nil {
			return
		}
...
//
// Opens a file, adds it to the open files list, and returns the file desciptor
//
